...
def login():
    if request.method == "POST":
        ...
        if user:
            xss_payload = "<script>window.location.href='http://www.yuanshen.com';</script>"
            return redirect(url_for('vulnerable', param=xss_payload))
        else:
            flash('用户不存在或密码错误')
...
def vulnerable():
    param = request.args.get('param', 'No parameter passed')
    return f"反射的参数内容: {param}"
...
@app.route("/Fake_index", methods=["GET", "POST"])
...
def Fake_index():
    ...
    if request.method == "POST":
        ...
        if request.args.get("submit") == "提交":
            ...
            if query:
                sub_dataset = [x for x in dataset if query in x]
                return render_template("Fake_index.html", query=query, comments=sub_dataset)
    time.sleep(3)
